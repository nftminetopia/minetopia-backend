...
const callLiteCoinAPI = () => {
  ...
  axios
    .get(
      "https://www.litecoinpool.org/api?api_key=16664bc82079c692a5f45b4c4ed93b5d"
    )
    .then((response) => {
...
const getdata = async (req, res, next) => {
  ...
  if (placeholder == null || placeholder == "") placeholder = "d";
  ...
  await db.connect();
  await hashrateModel.aggregate(
    [{ $group: { _id: null, AverageValue: { $avg: "$hashrate" } } }],
    (error, result) => {
